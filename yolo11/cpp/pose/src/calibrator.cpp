...
// This file is only used when INT8 is enabled for pose.
// The preprocess pipeline must match runtime behavior for the calibration cache to be meaningful.
...
std::vector<float> preprocess(cv::Mat& img, int input_w, int input_h)
{
    ...
    // Preserve the exact same letterbox rules as the pose inference pipeline.
    int w, h, x, y;
