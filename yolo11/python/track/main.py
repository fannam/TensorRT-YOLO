...
def make_parser():
    ...
    parser.add_argument("--detect-model",
                        type=str,
                        default=str(task_dir.parent / "detect" / "model.plan"),
                        help="Tensorrt plan path of YOLOv8 detection.")
    parser.add_argument("--video",
                        type=str,
                        default=str(task_dir.parents[1] / "assets" / "street.mp4"),
                        help="The path of the video to be tracked.")
    parser.add_argument("--output",
                        type=str,
                        default=str(task_dir / "output" / "result.mp4"),
                        help="The path of the tracking video output.")
    # tracking args
...
def main(args):
    from shared.python.tracker.byte_tracker import BYTETracker
    from yolo11.python.detect.infer import YoloDetector
    ...
    vid_writer = cv2.VideoWriter(str(output_path), cv2.VideoWriter_fourcc(*"mp4v"), fps, (int(width), int(height)))
