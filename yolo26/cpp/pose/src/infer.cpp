...
YoloDetector::YoloDetector(
    ...
    ): trtFile_(trtFile), onnxFile_(onnxFile)
{
    ...
    rawOutputMode_ = !(outDims.nbDims == 3 && outDims.d[1] <= kMaxNumOutputBbox && outDims.d[2] >= 5 + nk && outDims.d[2] <= 6 + nk);
    if (rawOutputMode_) {
        OUTPUT_CANDIDATES = outDims.d[2];
        outputRows_ = outDims.d[2];
        outputCols_ = outDims.d[1];
        kptOffset_ = 5;
    } else {
        OUTPUT_CANDIDATES = 0;
        outputRows_ = outDims.d[1];
        outputCols_ = outDims.d[2];
        kptOffset_ = (outputCols_ == 5 + nk) ? 5 : 6;
    }
    ...
    outputData = rawOutputMode_ ? new float[1 + kMaxNumOutputBbox * kNumBoxElement] : new float[outputRows_ * outputCols_];
...
YoloDetector::~YoloDetector(){
    ...
    if (transposeDevice != nullptr) CHECK(cudaFree(transposeDevice));
    if (decodeDevice != nullptr) CHECK(cudaFree(decodeDevice));
...
std::vector<Detection> YoloDetector::inference(cv::Mat& img){
    ...
    int nk = kNumKpt * kKptDims;
    if (rawOutputMode_) {
        transpose((float*)vBufferD[outputIndex_], transposeDevice, OUTPUT_CANDIDATES, 4 + kNumClass + nk, stream);
        ...
        for (int i = 0; i < count; i++){
            ...
            if (keepFlag == 1){
                ...
                memcpy(det.kpts, &outputData[pos + 7], nk * sizeof(float));
        ...
        for (int i = 0; i < outputRows_; i++) {
            ...
            if (detData[2] <= detData[0] || detData[3] <= detData[1]) continue;

            Detection det;
            det.bbox[0] = detData[0];
            det.bbox[1] = detData[1];
            det.bbox[2] = detData[2];
            det.bbox[3] = detData[3];
            det.conf = conf;
            det.classId = (kptOffset_ == 6) ? (int)detData[5] : 0;
            memcpy(det.kpts, &detData[kptOffset_], nk * sizeof(float));
            vDetections.push_back(det);
        }
    }
